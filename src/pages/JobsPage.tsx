...
import { collection, query, getDocs, addDoc, updateDoc, orderBy, Timestamp, deleteDoc, doc } from 'firebase/firestore';
import { db } from '../lib/firebase';
import { JobPost, OperationType } from '../types';
import { handleFirestoreError } from '../lib/firestoreUtils';
import { Phone, MapPin, Calendar, Users, Wallet, Briefcase, FileText, StickyNote, Plus, Trash2, Edit2, ExternalLink, Image as ImageIcon, Save, Search, X } from 'lucide-react';
  ...
  return (
    <div className="space-y-6 pt-0 pb-[15px]">
      <div className="pt-[5px] pb-[15px]">
        ...
        <h2 className="text-2xl font-black text-slate-900 tracking-tight">Bảng tin</h2>
              ...
              <div className="p-4 pt-[7px] pb-[7px] space-y-3">
                <div className="flex justify-between items-start mb-[5px]">
                  <div className="min-w-0 flex-1">
                    ...
                    {job.address && (
                      <p className="text-[10px] font-bold text-slate-400 mt-0.5 truncate uppercase tracking-wide">{job.address}</p>
                    )}
                    <div className="flex items-center gap-1.5 mt-0.5">
                      <span className="w-1.5 h-1.5 rounded-full bg-blue-500 animate-pulse"></span>
                      <span className="text-[9px] font-black text-blue-600 uppercase tracking-widest">Đang tuyển dụng</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-2">
                    {!isExpanded && (
                       <span className="text-[8px] font-black text-slate-400 uppercase tracking-widest bg-slate-50 px-2 py-1 rounded-full">Chi tiết</span>
                    )}
                    {profile?.role === 'admin' && (
                ...
                <div className="grid grid-cols-2 gap-2 mb-[5px]">
                  <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <p className="text-[7px] text-slate-400 font-black uppercase tracking-widest">Lương CB</p>
                    <p className="text-xs font-normal text-slate-900">{formatCurrency(job.baseSalary).replace('₫', '')} <span className="text-[7px] opacity-40 font-normal underline">đ</span></p>
                  </div>
                  <div className="bg-slate-50 p-2.5 rounded-xl border border-slate-100">
                    <p className="text-[7px] text-slate-400 font-black uppercase tracking-widest">Phụ cấp</p>
                    <p className="text-xs font-normal text-black">+{formatCurrency(job.allowance).replace('₫', '')} <span className="text-[7px] opacity-40 font-normal underline">đ</span></p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-2 gap-2 text-[10px] mb-[5px]">
                  <div className="flex items-center gap-1.5 text-slate-600 bg-slate-50/50 p-1.5 rounded-lg">
                    ...
                    <span className="truncate">PV: <span className="font-bold text-slate-900">{job.interviewTime || 'TBC'}</span></span>
                  </div>
                  <div className="flex items-center gap-1.5 text-slate-600 bg-slate-50/50 p-1.5 rounded-lg">
                    <Users size={12} className="text-indigo-500" />
                    <span className="truncate">Tuyển: <span className="font-bold text-slate-900">{job.gender}</span></span>
                  </div>
                </div>
                    ...
                    >
                      <div className="space-y-2">
                        <div className="bg-slate-50 p-3 rounded-xl border border-slate-100 relative overflow-hidden group/card shadow-inner">
                          <div className="flex items-center gap-2 text-[7px] font-black text-slate-400 uppercase tracking-widest mb-1"><FileText size={10} /> Giấy tờ hồ sơ</div>
                          <p className="text-[9px] text-slate-600 leading-relaxed font-medium">{job.requiredDocs || 'Liên hệ để biết thêm chi tiết'}</p>
                        </div>
                        {job.notes && (
                          <div className="bg-amber-50/50 p-3 rounded-xl border border-amber-100/50">
                            <div className="flex items-center gap-2 text-[7px] font-black text-amber-600 uppercase tracking-widest mb-1">Ghi chú</div>
                            <p className="text-[9px] text-slate-600 leading-relaxed font-medium">{job.notes}</p>
                          </div>
                        )}
                      </div>
                    </motion.div>
          ...
          <div className="text-center py-20 bg-gray-50 rounded-[2.5rem] border border-dashed border-slate-200">
            {searchTerm ? (
              <>
                ...
                <p className="text-slate-500 font-bold">Không tìm thấy kết quả cho "{searchTerm}"</p>
                <button onClick={() => setSearchTerm('')} className="mt-2 text-blue-600 text-sm font-black hover:underline">Xóa tìm kiếm</button>
              </>
            ) : (
              <>
                <Briefcase size={48} className="mx-auto text-slate-300 mb-4" />
                <p className="text-slate-400 font-medium">Hiện chưa có tin tuyển dụng nào.</p>
              </>
            )}
          </div>
        )}
